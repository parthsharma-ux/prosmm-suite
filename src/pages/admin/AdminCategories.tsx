...
export default function AdminCategories() {
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      ...
      if (error) toast.error(error.message); else { toast.success("Added"); setDialogOpen(false); fetch(); }
    }
  };

  const toggle = async (c: Category) => { await supabase.from("categories").update({ status: !c.status }).eq("id", c.id); fetch(); };
  const del = async (id: string) => { if (!confirm("Delete?")) return; await supabase.from("categories").delete().eq("id", id); toast.success("Deleted"); fetch(); };
